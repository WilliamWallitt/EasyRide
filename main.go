...
func main() {
	...
	// get driver (GET) - REMOVE
	// curl -v -X GET localhost:10000/drivers/{id}
	authRouter.HandleFunc("/drivers/{id}", Driver_Management_.GetDriverHandler).Methods("GET")

	// update driver (PUT)
	// curl -X PUT -H "Content-Type: application/json" -d '{"Rate":11}' http://localhost:1000/drivers
	authRouter.HandleFunc("/drivers", Driver_Management_.UpdateDriverHandler).Methods("PUT")
	...
	//curl -H "Content-Type: application/json" -X POST -d '{"Rate":11}' http://localhost:10000/rosters
	authRouter.HandleFunc("/rosters", Roster_Management_.AddDriverToRosterHandler).Methods("POST")

	// remove driver from roster (DELETE)
	//curl -X DELETE http://localhost:10000/rosters
	authRouter.HandleFunc("/rosters", Roster_Management_.RemoveDriverFromRosterHandler).Methods("DELETE")
